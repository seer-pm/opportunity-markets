...
import { Slide, Theme, ToastOptions, ToastPosition, toast } from 'react-toastify';
import {
...
const TX_CONFIRMATIONS = Number(import.meta.env.VITE_TX_CONFIRMATIONS ?? 0);
...
function toastContent(title: string, subtitle: string) {
  return (
    <div className="min-w-0 pr-5">
      <div className="font-display text-base font-semibold leading-tight tracking-tight text-paper">
        {title}
      </div>
      {subtitle ? (
        <div className="mt-1 font-sans text-base font-normal leading-snug text-muted">{subtitle}</div>
      ) : null}
    </div>
  );
}
