...
function getGenericMarketData(
    ...
    (acc, timestamp) => {
      const tokenPrices = poolsPairs.map(({ token1 }, tokenIndex) => {
        ...
        if (token0Price === '0' && token1Price === '0' && sqrtPrice && sqrtPrice !== '0') {
          const { token0Price: _token0Price, token1Price: _token1Price } =
            calculateTokenPricesFromSqrtPrice(sqrtPrice);
  ...
  const filteredTimestamps = timestamps.filter((x) => oddsMapping[String(x)]);
  ...
  if (getMarketType(market) === MarketTypes.SCALAR) {
    ...
    return { chartData, timestamps: filteredTimestamps };
  }
...
async function getFutarchyMarketData(
  ...
  const pricesMapping = timestamps.reduce(
    (acc, timestamp) => {
      ...
      acc[String(timestamp)] =
        Number.isNaN(yesPrice) || Number.isNaN(noPrice) ? null : [yesPrice, noPrice];
      return acc;
    },
    {} as { [key: string]: number[] | null }
  );
  ...
  if (
    timestamps.length > 0 &&
    pricesMapping?.[timestamps[0]]?.[0] === 0 &&
    pricesMapping?.[timestamps[0]]?.[1] === 0
  ) {
    const initialLiquidity = await Promise.all(
      poolsPairs.map((poolPair, tokenIndex) =>
        getInitialLiquidityPrice(market.chainId, poolPair, tokenIndex)
      )
    );
  ...
  const filteredTimestamps = timestamps.filter((x) => pricesMapping[String(x)]);
...
export async function filterChartData(
  ...
  try {
    ...
    const { firstTimestamp, lastTimestamp } = getFirstAndLastTimestamps(
      market,
      interval,
      dayCount,
      endDate
...
function filterChartDataByTimestamp(
  chartData: ChartData,
  ...
  const filteredChartData = chartData.chartData.map((series) => {
    const filteredData = series.data.filter(
      ([timestamp]) => timestamp >= firstTimestamp && timestamp <= lastTimestamp
    );
    return {
      ...series,
